...
from comtypes.gen.uAgent_Engine_Control import uAgentEngineControl as api
import configparser
import os, sys
import re
import subprocess
...
class Path(object):
    def __init__(self, config, base=os.environ["HOMEPATH"], extra=None):
        ...
        if not os.path.exists(zashel):
            os.mkdir(zashel)
        if not extra:
            finalpath = r"{}{}\Zashel\uAgentPy7.5".format(base, adicional)
        else:
            finalpath = r"{}{}\Zashel\uAgentPy7.5\{}".format(base, adicional, extra)
            if not os.path.exists(r"{}{}\Zashel\uAgentPy7.5".format(base, adicional)):
                os.mkdir(r"{}{}\Zashel\uAgentPy7.5".format(base, adicional))
...
class App(object):
    ...
    def call_direct(self, number):
        API.GlobalDial("={}".format(str(number)), "", "")
    ...
    def login(self, *, instance=None, username=None, password=None,
            setcontext=False, site=None, extension=None):
        global API
        '''
        ...
        '''
        if not username:
            if "username" in self.config.server: username = self.config.server["username"]
            else: raise Exception("Falta nombre de usuario")
        if not password:
            if "password" in self.config.server: password = self.config.server["password"]
            else: password = ""
        #API = comtypes.client.CreateObject(api)
        #Verificamos los datos para control de excepciones
        if not instance:
            if "instance" in self.config.server: instance = self.config.server["instance"]
            else: raise Exception("Falta nombre de la instancia")
        try:
            API.Login(instance, username, password)
        except:
            raise
        ...
        if setcontext:
            try:
                self.set_login_context(site=site,extension=extension)
            except:
                raise
    ...
    def set_login_context(self, site=None, extension=None):
        if not site:
            if "site" in self.config.server: site = self.config.server["site"]
            else: site="Madrid"
        if not extension:
...
class SqlParser(object):
    ...
    def sql(self, value):
        if not self.freezed:
            ...
            self._items = dict()
            try:
                self.cursorSQL = API.SqlCursorOpen(self.cursorSQL, self.sql, self.lastquery
            except:
    ...
    def close_cursor(self):
        if not self.freezed and self.cursorSQL != -1:
            API.SqlCursorClose(self.cursorSQL)
    ...
    def execute(self, sql, bind_list=()):
        if not self.freezed:
            for x in range(2):
                try:
                    self.cursorSQL = API.SqlCursorOpen(self.cursorSQL, sql, self.lastquery)
                    break
                except:
    ...
    def fetch_page(self, page, save=True):
        ...
        if not self.freezed and not page in self.items:
            ...
            data = API.SqlCursorFetch(self.cursorSQL, inicial, MAX_ROWS)
    ...
    def fetch_part(self, save=True):
        ...
        if not self.freezed:
            ...
            inicial = 0
            while total > 0:
                fetched = API.SqlCursorFetch(self.cursorSQL, inicial, MAX_ROWS)
        ...
        if not self.freezed:
            for table in table_name:
                ...
                if not "information_schema" in table:
                    ...
                    sql = "select column_name from {}information_schema.columns where table_name='{}'".format(db, tabled)
